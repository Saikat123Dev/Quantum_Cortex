...
- Web provider stores creds at `~/.quantumcortex/credentials/`; rerun `openclaw login` if logged out.
- Pi sessions live under `~/.quantumcortex/sessions/` by default; the base directory is not configurable.
...
- When asked to open a “session” file, open the Pi session logs under `~/.quantumcortex/agents/<agentId>/sessions/*.jsonl` (use the `agent=<id>` value in the Runtime line of the system prompt; newest unless a specific ID is given), not the default `sessions.json`. If logs are needed from another machine, SSH via Tailscale and read the same path there.
