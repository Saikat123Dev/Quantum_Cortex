...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Quantum_Cortex and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["quantumcortex update", "Update a source checkout (git)"],
        ["quantumcortex update --channel beta", "Switch to beta channel (git + npm)"],
        ["quantumcortex update --channel dev", "Switch to dev channel (git + npm)"],
        ["quantumcortex update --tag beta", "One-off update to a dist-tag or version"],
        ["quantumcortex update --dry-run", "Preview actions without changing anything"],
        ["quantumcortex update --no-restart", "Update without restarting the service"],
        ["quantumcortex update --json", "Output result as JSON"],
        ["quantumcortex update --yes", "Non-interactive (accept downgrade prompts)"],
        ["quantumcortex update wizard", "Interactive update wizard"],
        ["quantumcortex --update", "Shorthand for quantumcortex update"],
      ] as const;
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["quantumcortex update status", "Show channel + version status."],
          ["quantumcortex update status --json", "JSON output."],
          ["quantumcortex update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
