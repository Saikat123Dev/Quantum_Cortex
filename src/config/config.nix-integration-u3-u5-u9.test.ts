...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.quantumcortex when env not set", () => {
    ...
    it("CONFIG_PATH defaults to OPENCLAW_HOME/.quantumcortex/openclaw.json", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveConfigPathCandidate(
          envWith({
    ...
    it("CONFIG_PATH defaults to ~/.quantumcortex/openclaw.json when env not set", () => {
      expect(
        resolveConfigPathCandidate(
    ...
    it("CONFIG_PATH expands ~ in OPENCLAW_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        expect(
          resolveConfigPathCandidate(
            envWith({ OPENCLAW_HOME: home, OPENCLAW_CONFIG_PATH: "~/.quantumcortex/custom.json" }),
            () => home,
          ),
          ...
          JSON.stringify(
            {
                ...
                list: [
                  {
                    id: "main",
                    workspace: "~/ws-agent",
                    agentDir: "~/.quantumcortex/agents/main",
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.quantumcortex/credentials/wa-personal",
                    },
                  },
                },
              },
