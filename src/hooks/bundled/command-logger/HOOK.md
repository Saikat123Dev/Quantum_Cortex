...
2. **Appends to log file** - Writes a JSON line to `~/.quantumcortex/logs/commands.log`
...
`~/.quantumcortex/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.quantumcortex/logs/commands.log ~/.quantumcortex/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openclaw`:
   ```
   /home/username/.quantumcortex/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
