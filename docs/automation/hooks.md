...
- **💾 session-memory**: Saves session context to your agent workspace (default `~/.quantumcortex/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.quantumcortex/logs/commands.log`
...
2. **Managed hooks**: `~/.quantumcortex/hooks/` (user-installed, shared across workspaces)
...
Hook packs can ship dependencies; they will be installed under `~/.quantumcortex/hooks/<id>`.
...
- **Managed hooks** (`~/.quantumcortex/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.quantumcortex/hooks/my-hook
cd ~/.quantumcortex/hooks/my-hook
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.quantumcortex/workspace`)
...
**Output**: `~/.quantumcortex/logs/commands.log`
...
tail -n 20 ~/.quantumcortex/logs/commands.log

# Pretty-print with jq
cat ~/.quantumcortex/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.quantumcortex/logs/commands.log | jq .
```
...
# Other platforms
tail -f ~/.quantumcortex/gateway.log
```
...
1. Check directory structure:
   ...
   ```bash
   ls -la ~/.quantumcortex/hooks/my-hook/
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.quantumcortex/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.quantumcortex/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.quantumcortex/hooks/my-hook/handler.ts
   ```
