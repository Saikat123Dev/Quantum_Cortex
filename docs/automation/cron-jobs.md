...
- Jobs persist under `~/.quantumcortex/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.quantumcortex/cron/jobs.json` by default.
...
- Job store: `~/.quantumcortex/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.quantumcortex/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
```json5
{
  cron: {
    ...
    store: "~/.quantumcortex/cron/jobs.json",
