...
5. Configure `msteams` in `~/.quantumcortex/openclaw.json` (or env vars) and start the gateway.
...
- Votes are recorded by the gateway in `~/.quantumcortex/msteams-polls.json`.
