...
2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.quantumcortex`) from the old machine.
3. Copy your workspace (default: `~/.quantumcortex/workspace`).
...
under `~/.quantumcortex/` (for example `~/.quantumcortex/agents/<agentId>/sessions/`).
...
(`~/.quantumcortex`) and workspace (`~/.quantumcortex/workspace`) stay untouched.
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.quantumcortex/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.quantumcortex/openclaw.json`). Precedence is `<workspace>/skills` > `~/.quantumcortex/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.quantumcortex/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.quantumcortex/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which OpenClaw treats as `<workspace>/skills`.
...
3. Override the skill metadata (workspace or `~/.quantumcortex/skills`) to allow Linux:
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured OpenClaw workspace); OpenClaw treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.quantumcortex/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.quantumcortex` + your workspace directory).
...
Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.quantumcortex`):
...
Legacy single-agent path: `~/.quantumcortex/agent/*` (migrated by `openclaw doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.quantumcortex/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.quantumcortex`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.quantumcortex`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.quantumcortex/skills`).

Default workspace is `~/.quantumcortex/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.quantumcortex/workspace" } },
}
```
...
Do **not** commit anything under `~/.quantumcortex` (credentials, sessions, tokens, or encrypted secrets payloads).
...
OpenClaw reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.quantumcortex/openclaw.json`):

```
$OPENCLAW_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.quantumcortex/workspace`).
...
- Daemons read env vars from `~/.quantumcortex/.env` (or the service environment).
...
- Restore from backup (git or a copied `~/.quantumcortex/openclaw.json`).
...
- a global fallback `.env` from `~/.quantumcortex/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.quantumcortex/.env` so they're picked up even when the service doesn't inherit your shell env.
...
1. Put the token in `~/.quantumcortex/.env`:
...
- If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.quantumcortex-<profile>`).
...
- **Disk growth:** sessions + transcripts live under `~/.quantumcortex/agents/<agentId>/sessions/`.
...
- edit `agents.defaults.model` in `~/.quantumcortex/openclaw.json`
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.quantumcortex/agent/*` (migrated by `openclaw doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.quantumcortex/.env` or enable `env.shellEnv`.
...
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.quantumcortex/.env` on the **gateway host**.
...
- Use `openclaw --profile <name> …` per instance (auto-creates `~/.quantumcortex-<name>`).
...
- macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.quantumcortex/logs/...`; profiles use `~/.quantumcortex-<profile>/logs/...`)
