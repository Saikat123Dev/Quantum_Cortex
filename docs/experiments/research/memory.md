...
read_when:
  - Designing workspace memory (~/.quantumcortex/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.quantumcortex/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.quantumcortex/workspace` as canonical human-readable memory.

Suggested workspace layout:

```
~/.quantumcortex/workspace/
...
```
~/.quantumcortex/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.quantumcortex/workspace`:
