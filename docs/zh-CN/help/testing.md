...
- 配置文件存储：`~/.quantumcortex/credentials/`（首选；测试中"配置文件密钥"的含义）
...
- `OPENCLAW_CONFIG_DIR=...`（默认：`~/.quantumcortex`）挂载到 `/home/node/.quantumcortex`
- `OPENCLAW_WORKSPACE_DIR=...`（默认：`~/.quantumcortex/workspace`）挂载到 `/home/node/.quantumcortex/workspace`
