...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.quantumcortex-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.quantumcortex/logs/raw-stream.jsonl
```
...
OPENCLAW_RAW_STREAM_PATH=~/.quantumcortex/logs/raw-stream.jsonl
```

默认文件：

`~/.quantumcortex/logs/raw-stream.jsonl`
