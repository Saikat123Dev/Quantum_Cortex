...
1. 在新机器上安装 OpenClaw。
2. 从旧机器复制 `$OPENCLAW_STATE_DIR`（默认：`~/.quantumcortex`）。
3. 复制你的工作区（默认：`~/.quantumcortex/workspace`）。
4. 运行 `openclaw doctor` 并重启 Gateway 网关服务。

这会保留配置、认证配置文件、WhatsApp 凭据、会话和记忆。如果你处于远程模式，请记住 Gateway 网关主机拥有会话存储和工作区。

**重要：** 如果你只将工作区提交/推送到 GitHub，你只备份了**记忆 + 引导文件**，但**不包括**会话历史或认证。它们位于 `~/.quantumcortex/` 下（例如 `~/.quantumcortex/agents/<agentId>/sessions/`）。
...
（`~/.quantumcortex`）和工作区（`~/.quantumcortex/workspace`）保持不变。
...
使用托管覆盖而不是编辑仓库副本。将你的更改放在 `~/.quantumcortex/skills/<name>/SKILL.md`（或通过 `~/.quantumcortex/openclaw.json` 中的 `skills.load.extraDirs` 添加文件夹）。优先级是 `<workspace>/skills` > `~/.quantumcortex/skills` > 内置，所以托管覆盖优先生效而不会修改 git。只有值得上游合并的编辑才应该放在仓库中并作为 PR 提交。

### 可以从自定义文件夹加载 Skills 吗

可以。通过 `~/.quantumcortex/openclaw.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.quantumcortex/skills` → 内置 → `skills.load.extraDirs`。`clawhub` 默认安装到 `./skills`，OpenClaw 将其视为 `<workspace>/skills`。
...
3. 覆盖 Skills 元数据（工作区或 `~/.quantumcortex/skills`）以允许 Linux：
   ```markdown
   ---
   name: imsg
...
ClawHub 安装到当前目录下的 `./skills`（或回退到你配置的 OpenClaw 工作区）；OpenClaw 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.quantumcortex/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [ClawHub](/tools/clawhub)。
...
- **默认本地：** 会话、记忆文件、配置和工作区位于 Gateway 网关主机上（`~/.quantumcortex` + 你的工作区目录）。
...
所有内容位于 `$OPENCLAW_STATE_DIR`（默认：`~/.quantumcortex`）下：
...
旧版单智能体路径：`~/.quantumcortex/agent/*`（通过 `openclaw doctor` 迁移）。

你的**工作区**（AGENTS.md、记忆文件、Skills 等）是独立的，通过 `agents.defaults.workspace` 配置（默认：`~/.quantumcortex/workspace`）。

### AGENTS.md / SOUL.md / USER.md / MEMORY.md 应该放在哪里

这些文件位于**智能体工作区**中，而不是 `~/.quantumcortex`。

- **工作区（按智能体）**：`AGENTS.md`、`SOUL.md`、`IDENTITY.md`、`USER.md`、
  `MEMORY.md`（或 `memory.md`）、`memory/YYYY-MM-DD.md`、可选的 `HEARTBEAT.md`。
- **状态目录（`~/.quantumcortex`）**：配置、凭据、认证配置文件、会话、日志和共享 Skills（`~/.quantumcortex/skills`）。

默认工作区是 `~/.quantumcortex/workspace`，可通过以下方式配置：
...
**不要**提交 `~/.quantumcortex` 下的任何内容（凭据、会话、令牌）。如果你需要完整恢复，将工作区和状态目录分别备份（参阅上面的迁移问题）。
...
OpenClaw 从 `$OPENCLAW_CONFIG_PATH`（默认：`~/.quantumcortex/openclaw.json`）读取可选的 **JSON5** 配置：

```
$OPENCLAW_CONFIG_PATH
```

如果文件不存在，使用安全的默认值（包括默认工作区 `~/.quantumcortex/workspace`）。
...
- 守护进程从 `~/.quantumcortex/.env`（或服务环境）读取环境变量。
...
- 从备份恢复（git 或复制的 `~/.quantumcortex/openclaw.json`）。
...
- 当前工作目录下的 `.env`
- `~/.quantumcortex/.env`（即 `$OPENCLAW_STATE_DIR/.env`）的全局回退 `.env`
...
1. 将缺失的密钥放在 `~/.quantumcortex/.env` 中，这样即使服务不继承你的 shell 环境也能被获取。
...
- 如果你使用了配置文件（`--profile` / `OPENCLAW_PROFILE`），重置每个状态目录（默认为 `~/.quantumcortex-<profile>`）。
...
- **磁盘增长：** 会话 + 记录位于 `~/.quantumcortex/agents/<agentId>/sessions/` 下。
...
- 编辑 `~/.quantumcortex/openclaw.json` 中的 `agents.defaults.model`
...
- **确认认证配置文件的位置**（新路径 vs 旧路径）
  - 当前：`~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`
  - 旧版：`~/.quantumcortex/agent/*`（通过 `openclaw doctor` 迁移）
- **确认环境变量被 Gateway 网关加载**
  - 如果你在 shell 中设置了 `ANTHROPIC_API_KEY` 但通过 systemd/launchd 运行 Gateway 网关，它可能不会继承。将其放在 `~/.quantumcortex/.env` 中或启用 `env.shellEnv`。
- **确保你编辑的是正确的智能体**
...
- **如果你想使用 API 密钥**
  - 在 **Gateway 网关主机**上将 `ANTHROPIC_API_KEY` 放入 `~/.quantumcortex/.env`。
  - 清除任何强制缺失配置文件的固定顺序：
    ```bash
...
- 每实例使用 `openclaw --profile <name> …`（自动创建 `~/.quantumcortex-<name>`）。
...
- macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`（默认：`~/.quantumcortex/logs/...`；配置文件使用 `~/.quantumcortex-<profile>/logs/...`）
