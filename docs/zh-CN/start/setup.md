...
- **个性化设置存放在仓库之外：** `~/.quantumcortex/workspace`（工作区）+ `~/.quantumcortex/openclaw.json`（配置）。
...
- **配置：** `~/.quantumcortex/openclaw.json`（JSON/JSON5 格式）
- **工作区：** `~/.quantumcortex/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
- **配对允许列表**：`~/.quantumcortex/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.quantumcortex/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.quantumcortex/workspace` 和 `~/.quantumcortex/` 保持为"你的东西"；不要将个人提示/配置放入 `openclaw` 仓库。
