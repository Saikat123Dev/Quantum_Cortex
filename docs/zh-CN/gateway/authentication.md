...
3. 如果 Gateway 网关在 systemd/launchd 下运行，最好将密钥放在 `~/.quantumcortex/.env` 中以便守护进程可以读取：

```bash
cat >> ~/.quantumcortex/.env <<'EOF'
ANTHROPIC_API_KEY=...
EOF
```
...
参阅[帮助](/help)了解环境变量继承的详情（`env.shellEnv`、`~/.quantumcortex/.env`、systemd/launchd）。
