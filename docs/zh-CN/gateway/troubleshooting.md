...
变量如 `DISPLAY` 应该放在 `~/.quantumcortex/.env` 中（由 Gateway 网关早期加载）。
...
症状：`pwd` 或文件工具显示 `~/.quantumcortex/sandboxes/...` 即使你
期望的是主机工作区。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.quantumcortex/openclaw.json}"
```
...
```bash
ls -la ~/.quantumcortex/agents/<agentId>/sessions/
```
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.quantumcortex}/credentials" # 如果 logout 无法完全清除所有内容
...
```bash
# 在配置中打开跟踪日志：
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.quantumcortex/openclaw.json} -> { logging: { level: "trace" } }
#
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.quantumcortex/logs/...`；配置文件使用 `~/.quantumcortex-<profile>/logs/...`）<br />Linux：`journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "OpenClaw Gateway (<profile>)" /V /FO LIST` |
...
trash "${OPENCLAW_STATE_DIR:-$HOME/.quantumcortex}"
