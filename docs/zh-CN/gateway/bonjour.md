...
- 从 `~/.quantumcortex/dns/<domain>.db` 提供你选择的域名服务（示例：`openclaw.internal.`）
...
- 在 `~/.quantumcortex/openclaw.json` 中设置 `gateway.bind: "tailnet"`。
...
- `~/.quantumcortex/openclaw.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
