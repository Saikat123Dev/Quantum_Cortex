...
- 解释找到了哪些遗留键。
- 显示它应用的迁移。
- 使用更新后的 schema 重写 `~/.quantumcortex/openclaw.json`。
...
- 会话存储 + 记录：
  - 从 `~/.quantumcortex/sessions/` 到 `~/.quantumcortex/agents/<agentId>/sessions/`
- 智能体目录：
  - 从 `~/.quantumcortex/agent/` 到 `~/.quantumcortex/agents/<agentId>/agent/`
- WhatsApp 认证状态（Baileys）：
  - 从遗留的 `~/.quantumcortex/credentials/*.json`（除 `oauth.json` 外）
  - 到 `~/.quantumcortex/credentials/whatsapp/<accountId>/...`（默认账户 id：`default`）
...
- **多个状态目录**：当多个 `~/.quantumcortex` 文件夹存在于不同 home 目录或当 `OPENCLAW_STATE_DIR` 指向别处时发出警告（历史可能在安装之间分裂）。
- **远程模式提醒**：如果 `gateway.mode=remote`，doctor 会提醒你在远程主机上运行它（状态在那里）。
- **配置文件权限**：当 `~/.quantumcortex/openclaw.json` 对组/其他用户可读时发出警告，并提供收紧到 `600` 的选项。
