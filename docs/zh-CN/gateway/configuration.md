...
read_when:
  - 添加或修改配置字段时
summary: ~/.quantumcortex/openclaw.json 的所有配置选项及示例
title: 配置
x-i18n:
...
OpenClaw 从 `~/.quantumcortex/openclaw.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，OpenClaw 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.quantumcortex/workspace`）。通常只在以下情况需要配置：
...
请使用 `config.patch` 或 `openclaw config set`。请备份 `~/.quantumcortex/openclaw.json`。
...
{
  agents: {
    defaults: { workspace: "~/.quantumcortex/workspace" },
    list: [
      {
        id: "main",
...
```json5
// ~/.quantumcortex/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.quantumcortex/workspace" }],
}
```
...
```json5
// ~/.quantumcortex/clients/mueller/agents.json5
[
...
```json5
// ~/.quantumcortex/clients/mueller/broadcast.json5
{
...
- 当前工作目录中的 `.env`（如果存在）
- `~/.quantumcortex/.env`（即 `$OPENCLAW_STATE_DIR/.env`）作为全局回退 `.env`
...
- `<agentDir>/auth-profiles.json`（默认：`~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`）

另请参阅：[/concepts/oauth](/concepts/oauth)

旧版 OAuth 导入：

- `~/.quantumcortex/credentials/oauth.json`（或 `$OPENCLAW_STATE_DIR/credentials/oauth.json`）
...
- `~/.quantumcortex/agent/*`（由 `openclaw doctor` 迁移到 `~/.quantumcortex/agents/<defaultAgentId>/agent/*`）
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // 可选覆盖。默认：~/.quantumcortex/credentials/whatsapp/biz
          // authDir: "~/.quantumcortex/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- `agents.list[]`：每智能体覆盖。
  ...
  - `name`：智能体的显示名称。
  - `workspace`：默认 `~/.quantumcortex/workspace-<agentId>`（对于 `main`，回退到 `agents.defaults.workspace`）。
  - `agentDir`：默认 `~/.quantumcortex/agents/<agentId>/agent`。
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.quantumcortex/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.quantumcortex/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.quantumcortex/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.quantumcortex/workspace-home" },
      { id: "work", workspace: "~/.quantumcortex/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.quantumcortex/settings/tts.json",
      elevenlabs: {
...
- 智能体工作区访问：`workspaceAccess: "none"`（默认）
  - `"none"`：在 `~/.quantumcortex/sandboxes` 下使用每范围的沙箱工作区
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.quantumcortex/sandboxes",
        docker: {
...
`~/.quantumcortex/agents/<agentId>/agent/models.json` 或在 OpenClaw 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，OpenClaw 在启动时将 `models.json` 写入/合并到
`~/.quantumcortex/agents/<agentId>/agent/`：
...
- 如果你希望 `models.json` 存储在其他位置，请使用 `OPENCLAW_AGENT_DIR`（或 `PI_CODING_AGENT_DIR`）覆盖智能体配置根目录（默认：`~/.quantumcortex/agents/main/agent`）。
...
```json5
{
  session: {
    ...
    // 默认已按智能体存储在 ~/.quantumcortex/agents/<agentId>/sessions/sessions.json
    // 你可以使用 {agentId} 模板进行覆盖：
    store: "~/.quantumcortex/agents/{agentId}/sessions/sessions.json",
...
控制内置白名单、安装偏好、额外 Skills 文件夹和每 Skills 覆盖。适用于**内置**Skills 和 `~/.quantumcortex/skills`（工作区 Skills 在名称冲突时仍然优先）。
...
控制插件发现、允许/拒绝和每插件配置。插件从 `~/.quantumcortex/extensions`、`<workspace>/.quantumcortex/extensions` 以及任何 `plugins.load.paths` 条目加载。**配置更改需要重启 Gateway 网关。**
...
- OpenClaw.app 监视 `~/.quantumcortex/openclaw.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.quantumcortex/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.quantumcortex/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
...
- `openclaw --dev …` → 使用 `~/.quantumcortex-dev` + 端口从基础 `19001` 偏移
- `openclaw --profile <name> …` → 使用 `~/.quantumcortex-<name>`（端口通过配置/环境变量/标志）
...
```bash
OPENCLAW_CONFIG_PATH=~/.quantumcortex/a.json \
OPENCLAW_STATE_DIR=~/.quantumcortex-a \
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.quantumcortex/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
默认根目录：`~/.quantumcortex/workspace/canvas`
...
{
  canvasHost: {
    root: "~/.quantumcortex/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // 省略时使用 ~/.quantumcortex/bridge/tls/bridge-{cert,key}.pem。
      // certPath: "~/.quantumcortex/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.quantumcortex/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
启用后，Gateway 网关在 `~/.quantumcortex/dns/` 下使用配置的发现域（示例：`openclaw.internal.`）为 `_openclaw-gw._tcp` 写入单播 DNS-SD 区域。
