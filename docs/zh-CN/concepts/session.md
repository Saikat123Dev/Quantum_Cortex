...
- 在 **Gateway 网关主机**上：
  - 存储文件：`~/.quantumcortex/agents/<agentId>/sessions/sessions.json`（每个智能体）。
- 对话记录：`~/.quantumcortex/agents/<agentId>/sessions/<SessionId>.jsonl`（Telegram 话题会话使用 `.../<SessionId>-topic-<threadId>.jsonl`）。
