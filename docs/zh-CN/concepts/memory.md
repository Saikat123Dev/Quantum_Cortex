...
这些文件位于工作空间下（`agents.defaults.workspace`，默认 `~/.quantumcortex/workspace`）。完整布局参见[智能体工作空间](/concepts/agent-workspace)。
...
- 索引存储：每个智能体的 SQLite 位于 `~/.quantumcortex/memory/<agentId>.sqlite`（可通过 `agents.defaults.memorySearch.store.path` 配置，支持 `{agentId}` 令牌）。
...
- 会话日志存储在磁盘上（`~/.quantumcortex/agents/<agentId>/sessions/*.jsonl`）。任何具有文件系统访问权限的进程/用户都可以读取它们，因此将磁盘访问视为信任边界。对于更严格的隔离，在单独的操作系统用户或主机下运行智能体。
