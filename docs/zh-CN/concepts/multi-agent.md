...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.quantumcortex/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.quantumcortex/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 配置：`~/.quantumcortex/openclaw.json`（或 `OPENCLAW_CONFIG_PATH`）
- 状态目录：`~/.quantumcortex`（或 `OPENCLAW_STATE_DIR`）
- 工作区：`~/.quantumcortex/workspace`（或 `~/.quantumcortex/workspace-<agentId>`）
- 智能体目录：`~/.quantumcortex/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.quantumcortex/agents/<agentId>/sessions`
...
- 工作区默认为 `~/.quantumcortex/workspace`（或当设置了 `OPENCLAW_PROFILE` 时为 `~/.quantumcortex/workspace-<profile>`）。
- 状态默认为 `~/.quantumcortex/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.quantumcortex/credentials/whatsapp/personal
