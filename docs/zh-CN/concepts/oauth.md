...
- 认证配置文件（OAuth + API 密钥）：`~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`
- 运行时缓存（自动管理；请勿编辑）：`~/.quantumcortex/agents/<agentId>/agent/auth.json`

仅用于导入的旧版文件（仍然支持，但不是主存储）：

- `~/.quantumcortex/credentials/oauth.json`（首次使用时导入到 `auth-profiles.json`）
