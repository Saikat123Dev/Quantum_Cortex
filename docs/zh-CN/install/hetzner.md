...
- 在主机上持久化 `~/.quantumcortex` + `~/.quantumcortex/workspace`（重启/重建后保留）
...
| 组件             | 位置                                   | 持久化机制    | 说明                        |
| ---------------- | -------------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.quantumcortex/`           | 主机卷挂载    | 包括 `openclaw.json`、令牌  |
| 模型认证配置文件 | `/home/node/.quantumcortex/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.quantumcortex/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.quantumcortex/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.quantumcortex/`           | 主机卷挂载    | 保留二维码登录              |
| Gmail 密钥环     | `/home/node/.quantumcortex/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
