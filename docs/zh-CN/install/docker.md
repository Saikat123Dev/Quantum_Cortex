...
镜像以 `node`（uid 1000）运行。如果你在 `/home/node/.quantumcortex` 上看到权限错误，确保你的主机绑定挂载由 uid 1000 拥有。
...
- Gateway 网关容器是会话的真实来源（`~/.quantumcortex/agents/<agentId>/sessions/`）。
...
- 每个智能体一个容器
- 智能体工作区访问：`workspaceAccess: "none"`（默认）使用 `~/.quantumcortex/sandboxes`
