...
3. 获取机器人账户的访问令牌：
   ...
   - 或设置 `channels.matrix.userId` + `channels.matrix.password`：OpenClaw 会调用相同的登录端点，将访问令牌存储在 `~/.quantumcortex/credentials/matrix/credentials.json`，并在下次启动时重用。
...
加密状态按账户 + 访问令牌存储在 `~/.quantumcortex/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`（SQLite 数据库）。同步状态存储在同目录的 `bot-storage.json` 中。如果访问令牌（设备）更改，将创建新的存储，机器人必须重新验证才能访问加密房间。
