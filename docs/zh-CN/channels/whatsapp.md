...
2. 在 `~/.quantumcortex/openclaw.json` 中配置 WhatsApp。
...
- 凭证存储在 `~/.quantumcortex/credentials/whatsapp/<accountId>/creds.json`。
- 备份副本在 `creds.json.bak`（损坏时恢复）。
- 旧版兼容性：较旧的安装将 Baileys 文件直接存储在 `~/.quantumcortex/credentials/` 中。
