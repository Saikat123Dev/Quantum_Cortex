...
`~/.quantumcortex/exec-approvals.json`
...
- allowlist entries live in host-local `~/.quantumcortex/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `openclaw approvals allowlist ...`).
...
`~/.quantumcortex/exec-approvals.json` directly.
