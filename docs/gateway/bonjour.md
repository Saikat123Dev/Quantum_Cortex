...
- serve your chosen domain (example: `openclaw.internal.`) from `~/.quantumcortex/dns/<domain>.db`
...
- Set `gateway.bind: "tailnet"` in `~/.quantumcortex/openclaw.json`.
...
- `gateway.bind` in `~/.quantumcortex/openclaw.json` controls the Gateway bind mode.
