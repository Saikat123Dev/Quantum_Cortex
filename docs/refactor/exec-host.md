...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.quantumcortex/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.quantumcortex/node.json` (node id + pairing token).
...
Path: `~/.quantumcortex/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.quantumcortex/exec-approvals.sock",
...
- Unix socket at `~/.quantumcortex/exec-approvals.sock` (0600).
