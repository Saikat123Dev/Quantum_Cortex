...
<Steps>
  <Step title="Existing config detection">
    - If `~/.quantumcortex/openclaw.json` exists, choose Keep, Modify, or Reset.
...
- OAuth credentials: `~/.quantumcortex/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`
...
`~/.quantumcortex/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
