...
<Steps>
  <Step title="Existing config detection">
    - If `~/.quantumcortex/openclaw.json` exists, choose **Keep / Modify / Reset**.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.quantumcortex/credentials/oauth.json`; auth profiles live in `~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.quantumcortex/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.quantumcortex/workspace` (configurable).
...
- Stores it under `~/.quantumcortex/tools/signal-cli/<version>/`.
...
Typical fields in `~/.quantumcortex/openclaw.json`:
...
WhatsApp credentials go under `~/.quantumcortex/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.quantumcortex/agents/<agentId>/sessions/`.
