...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.quantumcortex/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.quantumcortex/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.quantumcortex/credentials/oauth.json` (imported into `auth-profiles.json` on first use)
