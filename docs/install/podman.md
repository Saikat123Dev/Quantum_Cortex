...
This also creates a minimal `~openclaw/.quantumcortex/openclaw.json` (sets `gateway.mode="local"`) so the gateway can start without running the wizard.
...
Then open `http://127.0.0.1:18789/` and use the token from `~openclaw/.quantumcortex/.env` (or the value printed by setup).
...
- **Home:** e.g. `/home/openclaw` — holds `~/.quantumcortex` (config, workspace) and the launch script `run-openclaw-podman.sh`.
...
- **Config:** Only `openclaw` and root can access `/home/openclaw/.quantumcortex`. To edit config: use the Control UI once the gateway is running, or `sudo -u openclaw $EDITOR /home/openclaw/.quantumcortex/openclaw.json`.

## Environment and config

- **Token:** Stored in `~openclaw/.quantumcortex/.env` as `OPENCLAW_GATEWAY_TOKEN`. `setup-podman.sh` and `run-openclaw-podman.sh` generate it if missing (uses `openssl`, `python3`, or `od`).
...
- **Paths:** Host config and workspace default to `~openclaw/.quantumcortex` and `~openclaw/.quantumcortex/workspace`. Override the host paths used by the launch script with `OPENCLAW_CONFIG_DIR` and `OPENCLAW_WORKSPACE_DIR`.
...
- **Gateway start blocked (missing `gateway.mode=local`):** Ensure `~openclaw/.quantumcortex/openclaw.json` exists and sets `gateway.mode="local"`. `setup-podman.sh` creates this file if missing.
