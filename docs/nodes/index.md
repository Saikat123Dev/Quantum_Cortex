...
- **Approvals**: enforced on the node host via `~/.quantumcortex/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.quantumcortex/openclaw.json` on the gateway host).
...
- `--display-name` on `openclaw node run` / `openclaw node install` (persists in `~/.quantumcortex/node.json` on the node).
...
Approvals live on the node host at `~/.quantumcortex/exec-approvals.json`.
...
- On headless node host, `system.run` is gated by exec approvals (`~/.quantumcortex/exec-approvals.json`).
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.quantumcortex/node.json`.
- Exec approvals are enforced locally via `~/.quantumcortex/exec-approvals.json`
