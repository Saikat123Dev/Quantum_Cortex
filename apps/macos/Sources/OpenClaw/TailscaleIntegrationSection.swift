...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.quantumcortex/openclaw.json. Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.quantumcortex/openclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.quantumcortex/openclaw.json. Restart the gateway to apply."
        }
