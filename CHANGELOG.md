...
- Docker/Setup: precreate `$OPENCLAW_CONFIG_DIR/identity` during `docker-setup.sh` so CLI commands that need device identity (for example `devices list`) avoid `EACCES ... /home/node/.quantumcortex/identity` failures on restrictive bind mounts. (#23948) Thanks @ackson-beep.
...
- Security/Hooks: restrict hook transform modules to `~/.quantumcortex/hooks/transforms` (prevents path traversal/escape module loads via config). Config note: `hooks.transformsDir` must now be within that directory. Thanks @akhmittra.
...
- Config/State: removed legacy `.moltbot` auto-detection/migration and `moltbot.json` config candidates. If you still have state/config under `~/.moltbot`, move it to `~/.quantumcortex` (recommended) or set `OPENCLAW_STATE_DIR` / `OPENCLAW_CONFIG_PATH` explicitly.
...
- Skills: update session-logs paths to use ~/.quantumcortex. (#4502) Thanks @bonald.
...
- Exec approvals: migrate approvals to `~/.quantumcortex/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.openclaw.ai/tools/exec-approvals
...
- **BREAKING:** `openclaw plugins install <path>` now copies into `~/.quantumcortex/extensions` (use `--link` to keep path-based loading).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.quantumcortex-dev`).
...
- Memory: new `openclaw memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.quantumcortex/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.quantumcortex/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Env: load global `$OPENCLAW_STATE_DIR/.env` (`~/.quantumcortex/.env`) as a fallback after CWD `.env`.
