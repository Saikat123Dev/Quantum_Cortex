...
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${CLAWDBOT_CONFIG_DIR:-$HOME/.quantumcortex}}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-${CLAWDBOT_WORKSPACE_DIR:-$HOME/.quantumcortex/workspace}}"
...
docker run --rm -t \
  ...
  -v "$CONFIG_DIR":/home/node/.quantumcortex \
  -v "$WORKSPACE_DIR":/home/node/.quantumcortex/workspace \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
